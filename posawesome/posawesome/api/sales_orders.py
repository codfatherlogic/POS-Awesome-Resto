...
def submit_sales_order(order):
    ...
    # Check if restaurant mode is enabled - skip Payment Entries for restaurant orders
    # as they handle payments directly through Sales Invoice
    is_restaurant_mode = False
    if so_doc.get("pos_profile"):
        is_restaurant_mode = frappe.db.get_value("POS Profile", so_doc.pos_profile, "posa_enable_restaurant_mode") or False

    if payments and not is_restaurant_mode:
        frappe.enqueue(
    ...
    # Payment entries run in the background to speed up checkout (except for restaurant orders)
