...
<template>
	<div class="pa-0">
			...
			<v-row align="start" no-gutters>
				<v-col cols="6">
					...
					>
						...
						<v-tooltip activator="parent" location="top" v-if="vaildatPayment">
							<span>Button disabled: {{ getDisabledReason() }}</span>
						</v-tooltip>
					</v-btn>
				</v-col>
		...
		<v-card v-if="showDebugInfo" flat class="cards mb-0 mt-2 pa-2" style="font-size: 12px;">
			<v-row dense>
				<v-col cols="12">
					<strong>Payment Debug Info:</strong><br>
					Invoice Type: {{ invoiceType }}<br>
					Invoice Doc Type: {{ invoice_doc?.doctype }}<br>
					Invoice Doc Name: {{ invoice_doc?.name }}<br>
					Is Return: {{ invoice_doc?.is_return }}<br>
					Restaurant Mode: {{ pos_profile?.posa_enable_restaurant_mode }}<br>
					Validation Result: {{ vaildatPayment }}<br>
					Loading: {{ loading }}<br>
					Grand Total: {{ invoice_doc?.grand_total }}<br>
					Rounded Total: {{ invoice_doc?.rounded_total }}<br>
					Delivery Date: {{ invoice_doc?.posa_delivery_date }}<br>
					Disabled Reason: {{ getDisabledReason() }}
				</v-col>
			</v-row>
			<v-btn size="small" @click="showDebugInfo = false" class="mt-2">Hide Debug</v-btn>
		</v-card>
...
export default {
	...
	computed: {
		...
		vaildatPayment() {
			// For restaurant mode, don't block payment submission based on delivery date
			if (this.pos_profile.posa_enable_restaurant_mode) {
				return false; // Always allow payment in restaurant mode
			}
			
			// For regular sales orders, check delivery date requirement
			if (this.pos_profile.posa_allow_sales_order) {
				if (this.invoiceType === "Order" && !this.invoice_doc.posa_delivery_date) {
					return true; // Block if delivery date is required but not set
				}
			}
			return false;
		},
	...
	methods: {
		...
		async submit(event, payment_received = false, print = false) {
			// MULTI-ORDER RESTAURANT WORKFLOW: Handle multiple draft Sales Orders
			if (this.invoice_doc.is_multi_order_edit && this.invoice_doc.multi_order_names) {
				...
				try {
					// Preserve payment amounts before processing
					const originalPayments = JSON.parse(JSON.stringify(this.invoice_doc.payments || []));
					console.log("Preserving payment amounts for multi-order submission:", originalPayments);
					
					// Step 1: Submit all draft orders and create consolidated invoice
					const multiOrderResult = await frappe.call({
					...
					if (multiOrderResult.message) {
						console.log("Successfully submitted multiple orders and created consolidated invoice:", multiOrderResult.message.name);
						
						// Update the current document to the new consolidated Sales Invoice
						this.invoice_doc = multiOrderResult.message;
						
						// Restore the payment amounts to the new invoice
			...
			// SINGLE ORDER RESTAURANT WORKFLOW: Handle draft Sales Orders
			else if (this.invoice_doc.doctype === "Sales Order" && this.invoice_doc.docstatus === 0) {
				console.log("Submitting draft restaurant order:", this.invoice_doc.name);
				
				try {
					// Step 1: Submit the draft Sales Order
					...
					if (submitResult.message) {
						console.log("Successfully submitted Sales Order:", submitResult.message.name);
						
						// CRITICAL: Preserve payment amounts before conversion
						const originalPayments = JSON.parse(JSON.stringify(this.invoice_doc.payments || []));
						console.log("Preserving original payment amounts:", originalPayments);
						
						// Step 2: Convert the submitted Sales Order to Sales Invoice for payment
						...
						if (invoiceResult.message) {
							...
							console.log("Successfully converted to Sales Invoice for payment:", this.invoice_doc.name);
							
							// CRITICAL FIX: Restore the original payment amounts to the new invoice
							if (originalPayments && originalPayments.length > 0) {
								console.log("Restoring payment amounts to new Sales Invoice...");
								
								// Map original payments to new invoice payment structure
								originalPayments.forEach(originalPayment => {
									// Find matching payment method in new invoice
									const matchingPayment = this.invoice_doc.payments.find(payment => 
										payment.mode_of_payment === originalPayment.mode_of_payment
									);
								...
								console.log("Payment amounts successfully restored to Sales Invoice");
							}
							...
							// Continue with the regular payment submission process below
						} else {
					...
					console.error("Error submitting draft restaurant order:", error);
					this.eventBus.emit("show_message", {
						title: __("Error submitting order: {0}", [error.message || "Unknown error"]),
						color: "error",
					});
					this.loading = false;
					return;
		...
		submit_invoice(print) {
			...
			frappe.call({
				...
				callback: function (r) {
					...
					vm.eventBus.emit("show_message", {
						title: vm.invoice_doc.doctype === "Sales Order"
		...
		getDisabledReason() {
			if (this.loading) return "Loading in progress";
			if (!this.invoice_doc) return "No invoice document";
			if (this.pos_profile.posa_enable_restaurant_mode) return "Restaurant mode - buttons should be enabled";
			if (this.pos_profile.posa_allow_sales_order && this.invoiceType === "Order" && !this.invoice_doc.posa_delivery_date) {
				return "Delivery date required for sales order";
			}
	...
	mounted() {
		this.$nextTick(() => {
			...
			this.eventBus.on("send_invoice_doc_payment", (invoice_doc) => {
				...
				// Update invoice type based on document type
				if (invoice_doc.doctype === "Sales Invoice" || invoice_doc.doctype === "POS Invoice") {
					this.invoiceType = "Invoice";
					console.log("Set invoice type to 'Invoice' based on doctype:", invoice_doc.doctype);
				} else if (invoice_doc.doctype === "Sales Order") {
					this.invoiceType = "Order";
					console.log("Set invoice type to 'Order' based on doctype:", invoice_doc.doctype);
				}
				...
				// Force UI update after setting up payment data
				this.$forceUpdate();
				console.log("Payments component setup complete, invoice type:", this.invoiceType);
			});
			...
			this.eventBus.on("update_invoice_type", (data) => {
				console.log("Payments component received invoice type update:", data);
				...
				// Force UI update to reflect invoice type change
				this.$forceUpdate();
				console.log("Payments component updated invoice type to:", this.invoiceType);
			});
			
			// Listen for change_invoice_type event (used by restaurant orders)
			this.eventBus.on("change_invoice_type", (type) => {
				console.log("Payments component received change_invoice_type:", type);
				this.invoiceType = type;
				
				// Force UI update to activate/deactivate buttons
				this.$forceUpdate();
				console.log("Payments component changed invoice type to:", this.invoiceType);
			});
